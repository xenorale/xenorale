# Владислав (xenorale) 👨‍💻

**Senior Python Developer | Architect | Automation Expert**

Работаю над тем, чтобы превращать сложную рутину в элегантную автоматизированность. Специализируюсь на проектировании отказоустойчивых систем, высоконагруженных API и интеллектуальных инструментов автоматизации.

---

### 🛠 Профессиональный стек:
*   **Backend:** Python (Expert), FastAPI, Django, Asyncio
*   **Databases:** PostgreSQL, Redis, MongoDB, SQLAlchemy
*   **Automation:** Playwright (Stealth), Selenium, Scrapy, Industrial-grade Web Scraping
*   **AI & ML:** Integration with LLMs (Grok, OpenAI, Anthropic), Computer Vision (OpenCV)
*   **Desktop & UI:** Flet (Glassmorphism), UI/UX for internal tools
*   **Infrastructure:** Git, Docker, CI/CD, Linux Server Management

---

### 📂 Избранные проекты:
*   **[Auto Grok Studio](https://github.com/xenorale/auto_gen_to_grok)** — High-end автоматизация генерации контента через Grok AI с визуальным контролем.
*   **[Dodo Table Detection](https://github.com/xenorale/dodo-table-detection)** — Интеллектуальный парсинг и детекция табличных данных (Computer Vision).
*   **[Student Help Bot](https://github.com/xenorale/stud_hlp_bot)** — Масштабируемый бот-помощник с глубокой интеграцией сервисов.

---

### 📫 Связь со мной:
<a href="https://t.me/antropolii" target="_blank">
  <img src="https://img.shields.io/badge/Telegram-26A5E4?style=flat-square&logo=telegram&logoColor=white" alt="Telegram">
</a>
<a href="mailto:v.d.obr@mail.ru">
  <img src="https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white" alt="Email">
</a>

---

### 📊 Статистика активности:
![Stats](https://github-readme-stats.vercel.app/api?username=xenorale&show_icons=true&theme=nord&hide_border=false&title_color=88C0D0&icon_color=88C0D0&text_color=D8DEE9&bg_color=2E3440)
![Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=xenorale&layout=compact&theme=nord&hide_border=false&title_color=88C0D0&text_color=D8DEE9&bg_color=2E3440)

<p align="left">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=xenorale&theme=nord&hide_border=false" alt="Streak Stats" />
</p>
